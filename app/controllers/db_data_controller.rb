class DbDataController < ApplicationController
  ...
  def insert_placement_rol_terms_of_appointment_sub_procedure
    # Offer
    procedure_steps = ProcedureStep.where(:name => "Offer");

    procedure_steps.each do |step|
      sub_step = ProcedureSubStep.where(:procedure_step_id => step.id, :identify_name => "role_terms_of_appointment");
      if (sub_step.blank?)
        ProcedureSubStep.create(:procedure_step_id => step.id,
                                :rank => 0,
                                :enabled => true,
                                :user_view => false,
                                :name => "Terms of Appointment",
                                :identify_name => "role_terms_of_appointment",
                                :t_start => step.t_start,
                                :t_end => step.t_end,
                                :url_acronym => "terms_appointment",
                                :lm_see => false
                              );
      end
    end
